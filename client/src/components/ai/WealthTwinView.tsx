...
const SPENDING_HISTORY = [65000, 68000, 72000, 70000, 75000, 72000, 69000, 71000, 74000, 72000, 68000, 72000];
...
const QUICK_QUESTIONS = [
  ...
  'How can I save more?',
  'Should I invest more in SIP?',
  'Analyze my spending',
];

const OFFLINE_RESPONSES: Record<string, string> = {
  '1 crore': 'At your current savings rate of 22.4%, you will reach ₹1 Crore by approximately 2031. Increasing your SIP by ₹2,000/month can get you there 8 months earlier!',
  'save more': 'You currently save 22.4% of income. Try the 50-30-20 rule: reduce discretionary spending and aim for 25% savings. That\'s an extra ₹3,250/month.',
  'sip': 'Yes! Based on your profile, increasing SIP by ₹2,000 adds ₹8.4L over 10 years at 12% returns. Consider Axis Bluechip or Nifty 50 Index.',
  'spending': 'Your top categories: Housing 35%, Shopping 17%, Food 15%. Dining out is 40% above average this week. Try a ₹2,000 weekly dining budget.',
  'default': 'I can help with projections, savings tips, SIP recommendations, tax planning, and spending analysis. What would you like to know?',
};
...
export default function WealthTwinView() {
  ...
  const totalExpenses = MOCK_SPENDING.reduce((sum, c) => sum + c.amount, 0);
  const topCategory = MOCK_SPENDING.reduce((max, c) => (c.amount > max.amount ? c : max), MOCK_SPENDING[0]);
  const avgMonthly = Math.round(SPENDING_HISTORY.reduce((a, b) => a + b, 0) / SPENDING_HISTORY.length);
  const trend = SPENDING_HISTORY[SPENDING_HISTORY.length - 1] > avgMonthly ? 'increasing' : 'stable';
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    {
      id: '0',
      role: 'bot',
      text: `Hello ${user.name}! I'm your Wealth Twin AI.\nI know your financial profile: Net Worth ₹${(currentNW / 1e5).toFixed(1)}L, saving ${savingsRate}% of income. Ask me about projections, savings tips, or goal planning!`,
    },
  ]);
  ...
  function sendChat(text: string) {
    if (!text.trim()) return;
    setChatMessages((m) => [...m, { id: Date.now().toString(), role: 'user', text }]);
    setChatInput('');
    setChatTyping(true);
    setTimeout(() => {
      ...
      for (const key of Object.keys(OFFLINE_RESPONSES)) {
        if (lower.includes(key)) { response = OFFLINE_RESPONSES[key]; break; }
      }
      response += '\n\n💡 Based on: Simulated user profile + Market data. Simulated outcome only. Not financial advice.';
      setChatMessages((m) => [...m, { id: (Date.now() + 1).toString(), role: 'bot', text: response }]);
      setChatTyping(false);
    }, 1000);
  }
    ...
    <div className="space-y-6 animate-fade-in pb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 card">
          <h3 className="font-semibold text-slate-800 dark:text-white mb-4">Net Worth Projection</h3>
          <div className="h-72">
      ...
      <div className="card overflow-hidden p-0">
        <div className="bg-gradient-to-r from-primary to-secondary p-4 text-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-white">Wealth Twin AI Chat</h3>
              <p className="text-xs text-white/70">Ask me anything about your finances</p>
            </div>
          </div>
          <span className="px-2 py-1 bg-white/20 rounded-full text-xs font-medium"><i className="fas fa-circle text-[8px] mr-1" />Gemini AI</span>
        </div>
        <div className="p-4">
          <div ref={chatContainerRef} className="space-y-3 mb-4 max-h-80 overflow-y-auto pr-1">
            {chatMessages.map((m) => (
              <div key={m.id} className={`flex items-start gap-2 ${m.role === 'user' ? 'justify-end' : ''}`}>
                {m.role === 'bot' && (
                  <div className="w-7 h-7 bg-primary/10 rounded-full flex items-center justify-center flex-shrink-0 text-primary text-xs">
                    <i className="fas fa-robot" />
                  </div>
                )}
                <div className={`p-3 text-sm rounded-2xl whitespace-pre-line max-w-[85%] ${m.role === 'user' ? 'bg-primary text-white rounded-tr-sm' : 'bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-200'}`}>
                  {m.text}
                </div>
              </div>
            ))}
            {chatTyping && (
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 bg-primary/10 rounded-full flex items-center justify-center text-primary text-xs"><i className="fas fa-robot" /></div>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {QUICK_QUESTIONS.map((q) => (
              <button key={q} onClick={() => sendChat(q)} className="px-3 py-1.5 bg-primary/10 text-primary text-xs rounded-full hover:bg-primary/20 transition-colors">
                {q}
              </button>
            ))}
          </div>
          <div className="flex gap-2">
            <input
              ...
              placeholder="Ask about your wealth, savings, investments..."
              className="flex-1 px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 dark:text-white"
            />
            <button onClick={() => sendChat(chatInput)} className="px-4 py-2 bg-primary text-white text-sm rounded-lg hover:bg-primary/90 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card">
          ...
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={MOCK_SPENDING} cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={3} dataKey="amount">
                  {MOCK_SPENDING.map((_, i) => (
