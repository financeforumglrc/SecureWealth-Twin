...
export default function FamilyDashboard() {
  const [activeTab, setActiveTab] = useState<'my' | 'family'>('family');
  ...
  const familyGoals = [
    { name: "Child's Education", target: 1500000, current: 780000, contributors: [{ name: 'Rahul', amount: 420000 }, { name: 'Priya', amount: 210000 }, { name: 'Rajesh', amount: 150000 }] },
    { name: 'Family Vacation', target: 300000, current: 95000, contributors: [{ name: 'Rahul', amount: 50000 }, { name: 'Priya', amount: 45000 }] },
    { name: 'Home Renovation', target: 800000, current: 320000, contributors: [{ name: 'Rahul', amount: 200000 }, { name: 'Priya', amount: 120000 }] },
  ];
    ...
    <div className="space-y-6">
        ...
        <div className="flex gap-1 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              activeTab === 'my' ? 'bg-white dark:bg-slate-700 text-primary shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            <i className="fas fa-user mr-1.5" /> My Wealth
          </button>
          <button
            onClick={() => setActiveTab('family')}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              activeTab === 'family' ? 'bg-white dark:bg-slate-700 text-primary shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            <i className="fas fa-people-group mr-1.5" /> Family Wealth
          </button>
        </div>
      ...
      {activeTab === 'my' ? (
        <div className="card text-center py-12">
          <p className="text-sm text-slate-500">Switch to the Dashboard view for your individual wealth details.</p>
        </div>
      ) : (
        <>
          {/* Combined Net Worth */}
